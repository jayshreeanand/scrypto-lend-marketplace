import { css as z, unsafeCSS as h, LitElement as G, html as g } from "lit";
import { customElement as P, property as f, state as bt } from "lit/decorators.js";
import { classMap as yt } from "lit/directives/class-map.js";
const at = {
  elementTag: "radix-connect-button",
  links: { "What is a radix wallet?": "https://wallet.radixdlt.com/" }
};
var mt = Object.defineProperty, xt = Object.getOwnPropertyDescriptor, Dt = (t, e, n, r) => {
  for (var o = r > 1 ? void 0 : r ? xt(e, n) : e, i = t.length - 1, a; i >= 0; i--)
    (a = t[i]) && (o = (r ? a(e, n, o) : a(o)) || o);
  return r && o && mt(e, n, o), o;
};
let H = class extends G {
  render() {
    return g`<div class="loader"></div>`;
  }
};
H.styles = z`
    :host {
      width: 1.5rem;
      height: 1.5rem;
      border: 3px solid #fff;
      border-left-color: rgba(255, 255, 255, 0.3);
      border-top-color: rgba(255, 255, 255, 0.3);
      border-bottom-color: rgba(255, 255, 255, 0.3);
      border-radius: 50%;
      display: inline-block;
      box-sizing: border-box;
      animation: rotation 1s linear infinite;
      align-self: center;
      margin-top: 0.1rem;
    }

    :host(.small) {
      width: 1rem;
      height: 1rem;
    }

    @keyframes rotation {
      0% {
        transform: rotate(0deg);
      }
      100% {
        transform: rotate(360deg);
      }
    }
  `;
H = Dt([
  P("loading-spinner")
], H);
const N = {
  radixBlue: z`#060F8F`,
  radixGrey1: z`#003057`,
  radixGrey4: z`#E2E5ED`,
  radixGrey5: z`#F4F5F9`
};
var zt = Object.defineProperty, Nt = Object.getOwnPropertyDescriptor, ht = (t, e, n, r) => {
  for (var o = r > 1 ? void 0 : r ? Nt(e, n) : e, i = t.length - 1, a; i >= 0; i--)
    (a = t[i]) && (o = (r ? a(e, n, o) : a(o)) || o);
  return r && o && zt(e, n, o), o;
};
let V = class extends G {
  render() {
    return g`<div class="wrapper">
      <div id="arrow"></div>
      <slot></slot>
    </div>`;
  }
};
V.styles = z`
    :host {
      width: 18rem;
      border-radius: 12px;
      position: absolute;
      background: ${N.radixGrey5};
      box-shadow: 0px 4px 9px rgba(0, 0, 0, 0.25);
      border-radius: 12px;
      border: 1px solid ${N.radixGrey4};
      padding: 1.13rem 1.13rem 0 1.13rem;
      z-index: 9999;
    }

    #arrow,
    #arrow::before {
      position: absolute;
      width: 8px;
      height: 8px;
      top: -0.21rem;
      right: 2rem;
      background: ${N.radixGrey5};
      border: 1px solid ${N.radixGrey4};
      border-bottom-width: 0;
      border-right-width: 0;
    }

    #arrow {
      visibility: hidden;
    }

    #arrow::before {
      visibility: visible;
      content: '';
      transform: rotate(45deg);
    }
  `;
V = ht([
  P("radix-popover")
], V);
const At = "data:image/svg+xml;base64,", vt = "data:image/svg+xml;base64,";
var It = Object.defineProperty, jt = Object.getOwnPropertyDescriptor, w = (t, e, n, r) => {
  for (var o = r > 1 ? void 0 : r ? jt(e, n) : e, i = t.length - 1, a; i >= 0; i--)
    (a = t[i]) && (o = (r ? a(e, n, o) : a(o)) || o);
  return r && o && It(e, n, o), o;
};
let A = class extends G {
  constructor() {
    super(...arguments), this.loading = !1, this.gradient = !1, this.logo = !1, this.fullWidth = !1, this.text = !1;
  }
  onClick(t) {
    this.dispatchEvent(
      new CustomEvent("onClick", {
        detail: t,
        bubbles: !0,
        composed: !0
      })
    );
  }
  render() {
    return g` <button
        class=${yt({
      logo: this.logo && !this.loading,
      gradient: this.gradient,
      "full-width": this.fullWidth,
      text: this.text
    })}
        part="button"
        @click=${this.onClick}
      >
        ${this.loading ? g`<loading-spinner />` : g`<slot></slot>`}</button
      ><slot></slot>`;
  }
};
A.styles = z`
    button {
      display: inline-block;
      height: 2.6rem;
      border-radius: 12px;
      border: 1px solid transparent;
      font-size: 1rem;
      font-weight: 500;
      font-family: inherit;
      transition: border-color 0.25s;
      background-color: ${N.radixBlue};
      color: white;
      text-shadow: 0px 0.7px 6px rgba(0, 0, 0, 0.7);
      background-color: ${N.radixBlue};
    }

    button.full-width {
      display: block;
      width: 100%;
    }

    :host(:not([disabled], .disabled)) > button {
      cursor: pointer;
    }

    button.logo::before {
      content: url(${h(vt)});
    }

    :host(.no-logo) > button::before {
      content: '';
    }

    button.gradient {
      background: url(${h(At)}) no-repeat;
      background-size: cover;
      width: 8.6rem;
    }

    button.text {
      background: none;
      color: ${N.radixBlue};
      text-shadow: 0px 0px 0px;
    }

    button:hover {
      border-color: none;
    }
    button:focus,
    button:focus-visible {
      outline: 0px auto -webkit-focus-ring-color;
    }
  `;
w([
  f({
    type: Boolean
  })
], A.prototype, "loading", 2);
w([
  f({
    type: Boolean
  })
], A.prototype, "gradient", 2);
w([
  f({
    type: Boolean
  })
], A.prototype, "logo", 2);
w([
  f({
    type: Boolean
  })
], A.prototype, "fullWidth", 2);
w([
  f({
    type: Boolean
  })
], A.prototype, "text", 2);
A = w([
  P("radix-button")
], A);
const wt = "data:image/svg+xml;base64,", Tt = "data:image/svg+xml;base64,", Zt = "data:image/svg+xml;base64,", Le = {
  pending: "pending",
  success: "success",
  fail: "fail"
}, B = {
  loginRequest: "loginRequest",
  dataRequest: "dataRequest",
  sendTransaction: "sendTransaction"
};
var Lt = Object.defineProperty, Ot = Object.getOwnPropertyDescriptor, X = (t, e, n, r) => {
  for (var o = r > 1 ? void 0 : r ? Ot(e, n) : e, i = t.length - 1, a; i >= 0; i--)
    (a = t[i]) && (o = (r ? a(e, n, o) : a(o)) || o);
  return r && o && Lt(e, n, o), o;
};
const Gt = {
  [B.dataRequest]: "Data request",
  [B.loginRequest]: "Login request",
  [B.sendTransaction]: "Send transaction"
};
let j = class extends G {
  constructor() {
    super(...arguments), this.features = { cancel: !1 };
  }
  onCancel() {
    var t;
    this.dispatchEvent(
      new CustomEvent("onCancel", {
        detail: { id: (t = this.item) == null ? void 0 : t.id },
        bubbles: !0,
        composed: !0
      })
    );
  }
  actionsTemplate() {
    var t, e;
    return this.item && ["dataRequest", "loginRequest"].includes(this.item.type) && this.item.status === "pending" && this.features.cancel ? g`<button @click=${this.onCancel} class="text cancel">
        Cancel
      </button>` : ((t = this.item) == null ? void 0 : t.status) === "fail" ? g`<span class="text error">${this.item.error}</span>` : ((e = this.item) == null ? void 0 : e.type) === "sendTransaction" && this.item.status === "success" && this.transactionExplorerBaseUrl ? g`<a
        target="_blank"
        href="${this.transactionExplorerBaseUrl}${this.item.transactionIntentHash}"
        class="text transaction"
        >${this.item.transactionIntentHash.slice(
      0,
      6
    )}...${this.item.transactionIntentHash.slice(-5)}</a
      >` : "";
  }
  render() {
    var t;
    return this.item ? g`<div class="wrapper main">
      <div class="wrapper type-status">
        <span class="icon ${(t = this.item) == null ? void 0 : t.status}"></span>
        <span class="text type"
          >${Gt[this.item.type]}</span
        >
      </div>
      ${this.actionsTemplate()}
    </div>` : "";
  }
};
j.styles = z`
    :host {
      background: #e2e5ed;
      display: flex;
      width: 100%;
      padding: 0.5em 1em;
      border-radius: 8px;
      box-sizing: border-box;
      height: 2rem;
    }
    a,
    a:visited {
      color: #003057;
    }
    .wrapper.main {
      display: flex;
      width: 100%;
      justify-content: space-between;
    }
    .wrapper.type-status {
      display: flex;
      align-self: center;
    }
    .text.type {
      display: inline-block;
      font-size: 0.9rem;
      line-height: 1rem;
      margin-left: 0.75rem;
      white-space: nowrap;
    }
    .text.request:first-letter {
      text-transform: uppercase;
    }
    .text.cancel {
      align-self: center;
      cursor: pointer;
      font-size: 0.75rem;
      border: 1px solid grey;
      border-radius: 8px;
      padding: 0.1rem 0.4rem;
      transition: background-color 0.1s ease;
    }
    .text.error {
      align-self: center;
      font-size: 0.75rem;
      margin-left: 1rem;
      text-overflow: ellipsis;
      white-space: nowrap;
      overflow: hidden;
    }
    .text.transaction {
      align-self: center;
      font-size: 0.75rem;
    }
    .text.cancel:hover {
      background: rgba(0, 0, 0, 0.1);
    }
    .text.status {
      text-transform: capitalize;
      font-weight: 500;
    }
    .icon {
      align-self: center;
      height: 1rem;
    }
    .icon.pending::before {
      content: url(${h(wt)});
    }
    .icon.success::before {
      content: url(${h(Tt)});
    }
    .icon.fail::before {
      content: url(${h(Zt)});
      position: relative;
      top: -0.1rem;
    }
  `;
X([
  f({
    type: Object
  })
], j.prototype, "item", 2);
X([
  f({
    type: String
  })
], j.prototype, "transactionExplorerBaseUrl", 2);
X([
  bt()
], j.prototype, "features", 2);
j = X([
  P("radix-request-item")
], j);
const Pt = "data:image/png;base64,", Et = "data:image/svg+xml;base64,", St = "data:image/svg+xml;base64,", Wt = "data:image/svg+xml;base64,";
var U = function(t, e) {
  return U = Object.setPrototypeOf || { __proto__: [] } instanceof Array && function(n, r) {
    n.__proto__ = r;
  } || function(n, r) {
    for (var o in r)
      Object.prototype.hasOwnProperty.call(r, o) && (n[o] = r[o]);
  }, U(t, e);
};
function F(t, e) {
  if (typeof e != "function" && e !== null)
    throw new TypeError("Class extends value " + String(e) + " is not a constructor or null");
  U(t, e);
  function n() {
    this.constructor = t;
  }
  t.prototype = e === null ? Object.create(e) : (n.prototype = e.prototype, new n());
}
function Xt(t, e, n, r) {
  function o(i) {
    return i instanceof n ? i : new n(function(a) {
      a(i);
    });
  }
  return new (n || (n = Promise))(function(i, a) {
    function u(l) {
      try {
        c(r.next(l));
      } catch (m) {
        a(m);
      }
    }
    function s(l) {
      try {
        c(r.throw(l));
      } catch (m) {
        a(m);
      }
    }
    function c(l) {
      l.done ? i(l.value) : o(l.value).then(u, s);
    }
    c((r = r.apply(t, e || [])).next());
  });
}
function ct(t, e) {
  var n = { label: 0, sent: function() {
    if (i[0] & 1)
      throw i[1];
    return i[1];
  }, trys: [], ops: [] }, r, o, i, a;
  return a = { next: u(0), throw: u(1), return: u(2) }, typeof Symbol == "function" && (a[Symbol.iterator] = function() {
    return this;
  }), a;
  function u(c) {
    return function(l) {
      return s([c, l]);
    };
  }
  function s(c) {
    if (r)
      throw new TypeError("Generator is already executing.");
    for (; a && (a = 0, c[0] && (n = 0)), n; )
      try {
        if (r = 1, o && (i = c[0] & 2 ? o.return : c[0] ? o.throw || ((i = o.return) && i.call(o), 0) : o.next) && !(i = i.call(o, c[1])).done)
          return i;
        switch (o = 0, i && (c = [c[0] & 2, i.value]), c[0]) {
          case 0:
          case 1:
            i = c;
            break;
          case 4:
            return n.label++, { value: c[1], done: !1 };
          case 5:
            n.label++, o = c[1], c = [0];
            continue;
          case 7:
            c = n.ops.pop(), n.trys.pop();
            continue;
          default:
            if (i = n.trys, !(i = i.length > 0 && i[i.length - 1]) && (c[0] === 6 || c[0] === 2)) {
              n = 0;
              continue;
            }
            if (c[0] === 3 && (!i || c[1] > i[0] && c[1] < i[3])) {
              n.label = c[1];
              break;
            }
            if (c[0] === 6 && n.label < i[1]) {
              n.label = i[1], i = c;
              break;
            }
            if (i && n.label < i[2]) {
              n.label = i[2], n.ops.push(c);
              break;
            }
            i[2] && n.ops.pop(), n.trys.pop();
            continue;
        }
        c = e.call(t, n);
      } catch (l) {
        c = [6, l], o = 0;
      } finally {
        r = i = 0;
      }
    if (c[0] & 5)
      throw c[1];
    return { value: c[0] ? c[1] : void 0, done: !0 };
  }
}
function Z(t) {
  var e = typeof Symbol == "function" && Symbol.iterator, n = e && t[e], r = 0;
  if (n)
    return n.call(t);
  if (t && typeof t.length == "number")
    return {
      next: function() {
        return t && r >= t.length && (t = void 0), { value: t && t[r++], done: !t };
      }
    };
  throw new TypeError(e ? "Object is not iterable." : "Symbol.iterator is not defined.");
}
function L(t, e) {
  var n = typeof Symbol == "function" && t[Symbol.iterator];
  if (!n)
    return t;
  var r = n.call(t), o, i = [], a;
  try {
    for (; (e === void 0 || e-- > 0) && !(o = r.next()).done; )
      i.push(o.value);
  } catch (u) {
    a = { error: u };
  } finally {
    try {
      o && !o.done && (n = r.return) && n.call(r);
    } finally {
      if (a)
        throw a.error;
    }
  }
  return i;
}
function W(t, e, n) {
  if (n || arguments.length === 2)
    for (var r = 0, o = e.length, i; r < o; r++)
      (i || !(r in e)) && (i || (i = Array.prototype.slice.call(e, 0, r)), i[r] = e[r]);
  return t.concat(i || Array.prototype.slice.call(e));
}
function I(t) {
  return this instanceof I ? (this.v = t, this) : new I(t);
}
function Ct(t, e, n) {
  if (!Symbol.asyncIterator)
    throw new TypeError("Symbol.asyncIterator is not defined.");
  var r = n.apply(t, e || []), o, i = [];
  return o = {}, a("next"), a("throw"), a("return"), o[Symbol.asyncIterator] = function() {
    return this;
  }, o;
  function a(M) {
    r[M] && (o[M] = function(b) {
      return new Promise(function(D, p) {
        i.push([M, b, D, p]) > 1 || u(M, b);
      });
    });
  }
  function u(M, b) {
    try {
      s(r[M](b));
    } catch (D) {
      m(i[0][3], D);
    }
  }
  function s(M) {
    M.value instanceof I ? Promise.resolve(M.value.v).then(c, l) : m(i[0][2], M);
  }
  function c(M) {
    u("next", M);
  }
  function l(M) {
    u("throw", M);
  }
  function m(M, b) {
    M(b), i.shift(), i.length && u(i[0][0], i[0][1]);
  }
}
function Yt(t) {
  if (!Symbol.asyncIterator)
    throw new TypeError("Symbol.asyncIterator is not defined.");
  var e = t[Symbol.asyncIterator], n;
  return e ? e.call(t) : (t = typeof Z == "function" ? Z(t) : t[Symbol.iterator](), n = {}, r("next"), r("throw"), r("return"), n[Symbol.asyncIterator] = function() {
    return this;
  }, n);
  function r(i) {
    n[i] = t[i] && function(a) {
      return new Promise(function(u, s) {
        a = t[i](a), o(u, s, a.done, a.value);
      });
    };
  }
  function o(i, a, u, s) {
    Promise.resolve(s).then(function(c) {
      i({ value: c, done: u });
    }, a);
  }
}
function d(t) {
  return typeof t == "function";
}
function Bt(t) {
  var e = function(r) {
    Error.call(r), r.stack = new Error().stack;
  }, n = t(e);
  return n.prototype = Object.create(Error.prototype), n.prototype.constructor = n, n;
}
var k = Bt(function(t) {
  return function(e) {
    t(this), this.message = e ? e.length + ` errors occurred during unsubscription:
` + e.map(function(n, r) {
      return r + 1 + ") " + n.toString();
    }).join(`
  `) : "", this.name = "UnsubscriptionError", this.errors = e;
  };
});
function tt(t, e) {
  if (t) {
    var n = t.indexOf(e);
    0 <= n && t.splice(n, 1);
  }
}
var C = function() {
  function t(e) {
    this.initialTeardown = e, this.closed = !1, this._parentage = null, this._finalizers = null;
  }
  return t.prototype.unsubscribe = function() {
    var e, n, r, o, i;
    if (!this.closed) {
      this.closed = !0;
      var a = this._parentage;
      if (a)
        if (this._parentage = null, Array.isArray(a))
          try {
            for (var u = Z(a), s = u.next(); !s.done; s = u.next()) {
              var c = s.value;
              c.remove(this);
            }
          } catch (p) {
            e = { error: p };
          } finally {
            try {
              s && !s.done && (n = u.return) && n.call(u);
            } finally {
              if (e)
                throw e.error;
            }
          }
        else
          a.remove(this);
      var l = this.initialTeardown;
      if (d(l))
        try {
          l();
        } catch (p) {
          i = p instanceof k ? p.errors : [p];
        }
      var m = this._finalizers;
      if (m) {
        this._finalizers = null;
        try {
          for (var M = Z(m), b = M.next(); !b.done; b = M.next()) {
            var D = b.value;
            try {
              et(D);
            } catch (p) {
              i = i ?? [], p instanceof k ? i = W(W([], L(i)), L(p.errors)) : i.push(p);
            }
          }
        } catch (p) {
          r = { error: p };
        } finally {
          try {
            b && !b.done && (o = M.return) && o.call(M);
          } finally {
            if (r)
              throw r.error;
          }
        }
      }
      if (i)
        throw new k(i);
    }
  }, t.prototype.add = function(e) {
    var n;
    if (e && e !== this)
      if (this.closed)
        et(e);
      else {
        if (e instanceof t) {
          if (e.closed || e._hasParent(this))
            return;
          e._addParent(this);
        }
        (this._finalizers = (n = this._finalizers) !== null && n !== void 0 ? n : []).push(e);
      }
  }, t.prototype._hasParent = function(e) {
    var n = this._parentage;
    return n === e || Array.isArray(n) && n.includes(e);
  }, t.prototype._addParent = function(e) {
    var n = this._parentage;
    this._parentage = Array.isArray(n) ? (n.push(e), n) : n ? [n, e] : e;
  }, t.prototype._removeParent = function(e) {
    var n = this._parentage;
    n === e ? this._parentage = null : Array.isArray(n) && tt(n, e);
  }, t.prototype.remove = function(e) {
    var n = this._finalizers;
    n && tt(n, e), e instanceof t && e._removeParent(this);
  }, t.EMPTY = function() {
    var e = new t();
    return e.closed = !0, e;
  }(), t;
}();
C.EMPTY;
function st(t) {
  return t instanceof C || t && "closed" in t && d(t.remove) && d(t.add) && d(t.unsubscribe);
}
function et(t) {
  d(t) ? t() : t.unsubscribe();
}
var ut = {
  onUnhandledError: null,
  onStoppedNotification: null,
  Promise: void 0,
  useDeprecatedSynchronousErrorHandling: !1,
  useDeprecatedNextContext: !1
}, kt = {
  setTimeout: function(t, e) {
    for (var n = [], r = 2; r < arguments.length; r++)
      n[r - 2] = arguments[r];
    return setTimeout.apply(void 0, W([t, e], L(n)));
  },
  clearTimeout: function(t) {
    return clearTimeout(t);
  },
  delegate: void 0
};
function lt(t) {
  kt.setTimeout(function() {
    throw t;
  });
}
function nt() {
}
function Qt(t) {
  t();
}
var q = function(t) {
  F(e, t);
  function e(n) {
    var r = t.call(this) || this;
    return r.isStopped = !1, n ? (r.destination = n, st(n) && n.add(r)) : r.destination = Jt, r;
  }
  return e.create = function(n, r, o) {
    return new J(n, r, o);
  }, e.prototype.next = function(n) {
    this.isStopped || this._next(n);
  }, e.prototype.error = function(n) {
    this.isStopped || (this.isStopped = !0, this._error(n));
  }, e.prototype.complete = function() {
    this.isStopped || (this.isStopped = !0, this._complete());
  }, e.prototype.unsubscribe = function() {
    this.closed || (this.isStopped = !0, t.prototype.unsubscribe.call(this), this.destination = null);
  }, e.prototype._next = function(n) {
    this.destination.next(n);
  }, e.prototype._error = function(n) {
    try {
      this.destination.error(n);
    } finally {
      this.unsubscribe();
    }
  }, e.prototype._complete = function() {
    try {
      this.destination.complete();
    } finally {
      this.unsubscribe();
    }
  }, e;
}(C), Ht = Function.prototype.bind;
function Q(t, e) {
  return Ht.call(t, e);
}
var Vt = function() {
  function t(e) {
    this.partialObserver = e;
  }
  return t.prototype.next = function(e) {
    var n = this.partialObserver;
    if (n.next)
      try {
        n.next(e);
      } catch (r) {
        S(r);
      }
  }, t.prototype.error = function(e) {
    var n = this.partialObserver;
    if (n.error)
      try {
        n.error(e);
      } catch (r) {
        S(r);
      }
    else
      S(e);
  }, t.prototype.complete = function() {
    var e = this.partialObserver;
    if (e.complete)
      try {
        e.complete();
      } catch (n) {
        S(n);
      }
  }, t;
}(), J = function(t) {
  F(e, t);
  function e(n, r, o) {
    var i = t.call(this) || this, a;
    if (d(n) || !n)
      a = {
        next: n ?? void 0,
        error: r ?? void 0,
        complete: o ?? void 0
      };
    else {
      var u;
      i && ut.useDeprecatedNextContext ? (u = Object.create(n), u.unsubscribe = function() {
        return i.unsubscribe();
      }, a = {
        next: n.next && Q(n.next, u),
        error: n.error && Q(n.error, u),
        complete: n.complete && Q(n.complete, u)
      }) : a = n;
    }
    return i.destination = new Vt(a), i;
  }
  return e;
}(q);
function S(t) {
  lt(t);
}
function Ut(t) {
  throw t;
}
var Jt = {
  closed: !0,
  next: nt,
  error: Ut,
  complete: nt
}, K = function() {
  return typeof Symbol == "function" && Symbol.observable || "@@observable";
}();
function dt(t) {
  return t;
}
function Rt(t) {
  return t.length === 0 ? dt : t.length === 1 ? t[0] : function(e) {
    return t.reduce(function(n, r) {
      return r(n);
    }, e);
  };
}
var v = function() {
  function t(e) {
    e && (this._subscribe = e);
  }
  return t.prototype.lift = function(e) {
    var n = new t();
    return n.source = this, n.operator = e, n;
  }, t.prototype.subscribe = function(e, n, r) {
    var o = this, i = qt(e) ? e : new J(e, n, r);
    return Qt(function() {
      var a = o, u = a.operator, s = a.source;
      i.add(u ? u.call(i, s) : s ? o._subscribe(i) : o._trySubscribe(i));
    }), i;
  }, t.prototype._trySubscribe = function(e) {
    try {
      return this._subscribe(e);
    } catch (n) {
      e.error(n);
    }
  }, t.prototype.forEach = function(e, n) {
    var r = this;
    return n = rt(n), new n(function(o, i) {
      var a = new J({
        next: function(u) {
          try {
            e(u);
          } catch (s) {
            i(s), a.unsubscribe();
          }
        },
        error: i,
        complete: o
      });
      r.subscribe(a);
    });
  }, t.prototype._subscribe = function(e) {
    var n;
    return (n = this.source) === null || n === void 0 ? void 0 : n.subscribe(e);
  }, t.prototype[K] = function() {
    return this;
  }, t.prototype.pipe = function() {
    for (var e = [], n = 0; n < arguments.length; n++)
      e[n] = arguments[n];
    return Rt(e)(this);
  }, t.prototype.toPromise = function(e) {
    var n = this;
    return e = rt(e), new e(function(r, o) {
      var i;
      n.subscribe(function(a) {
        return i = a;
      }, function(a) {
        return o(a);
      }, function() {
        return r(i);
      });
    });
  }, t.create = function(e) {
    return new t(e);
  }, t;
}();
function rt(t) {
  var e;
  return (e = t ?? ut.Promise) !== null && e !== void 0 ? e : Promise;
}
function Ft(t) {
  return t && d(t.next) && d(t.error) && d(t.complete);
}
function qt(t) {
  return t && t instanceof q || Ft(t) && st(t);
}
function Kt(t) {
  return d(t == null ? void 0 : t.lift);
}
function Y(t) {
  return function(e) {
    if (Kt(e))
      return e.lift(function(n) {
        try {
          return t(n, this);
        } catch (r) {
          this.error(r);
        }
      });
    throw new TypeError("Unable to lift unknown Observable type");
  };
}
function O(t, e, n, r, o) {
  return new $t(t, e, n, r, o);
}
var $t = function(t) {
  F(e, t);
  function e(n, r, o, i, a, u) {
    var s = t.call(this, n) || this;
    return s.onFinalize = a, s.shouldUnsubscribe = u, s._next = r ? function(c) {
      try {
        r(c);
      } catch (l) {
        n.error(l);
      }
    } : t.prototype._next, s._error = i ? function(c) {
      try {
        i(c);
      } catch (l) {
        n.error(l);
      } finally {
        this.unsubscribe();
      }
    } : t.prototype._error, s._complete = o ? function() {
      try {
        o();
      } catch (c) {
        n.error(c);
      } finally {
        this.unsubscribe();
      }
    } : t.prototype._complete, s;
  }
  return e.prototype.unsubscribe = function() {
    var n;
    if (!this.shouldUnsubscribe || this.shouldUnsubscribe()) {
      var r = this.closed;
      t.prototype.unsubscribe.call(this), !r && ((n = this.onFinalize) === null || n === void 0 || n.call(this));
    }
  }, e;
}(q), Mt = function(t) {
  return t && typeof t.length == "number" && typeof t != "function";
};
function _t(t) {
  return d(t == null ? void 0 : t.then);
}
function te(t) {
  return d(t[K]);
}
function ee(t) {
  return Symbol.asyncIterator && d(t == null ? void 0 : t[Symbol.asyncIterator]);
}
function ne(t) {
  return new TypeError("You provided " + (t !== null && typeof t == "object" ? "an invalid object" : "'" + t + "'") + " where a stream was expected. You can provide an Observable, Promise, ReadableStream, Array, AsyncIterable, or Iterable.");
}
function re() {
  return typeof Symbol != "function" || !Symbol.iterator ? "@@iterator" : Symbol.iterator;
}
var ie = re();
function oe(t) {
  return d(t == null ? void 0 : t[ie]);
}
function ae(t) {
  return Ct(this, arguments, function() {
    var e, n, r, o;
    return ct(this, function(i) {
      switch (i.label) {
        case 0:
          e = t.getReader(), i.label = 1;
        case 1:
          i.trys.push([1, , 9, 10]), i.label = 2;
        case 2:
          return [4, I(e.read())];
        case 3:
          return n = i.sent(), r = n.value, o = n.done, o ? [4, I(void 0)] : [3, 5];
        case 4:
          return [2, i.sent()];
        case 5:
          return [4, I(r)];
        case 6:
          return [4, i.sent()];
        case 7:
          return i.sent(), [3, 2];
        case 8:
          return [3, 10];
        case 9:
          return e.releaseLock(), [7];
        case 10:
          return [2];
      }
    });
  });
}
function ce(t) {
  return d(t == null ? void 0 : t.getReader);
}
function $(t) {
  if (t instanceof v)
    return t;
  if (t != null) {
    if (te(t))
      return se(t);
    if (Mt(t))
      return ue(t);
    if (_t(t))
      return le(t);
    if (ee(t))
      return gt(t);
    if (oe(t))
      return de(t);
    if (ce(t))
      return Me(t);
  }
  throw ne(t);
}
function se(t) {
  return new v(function(e) {
    var n = t[K]();
    if (d(n.subscribe))
      return n.subscribe(e);
    throw new TypeError("Provided object does not correctly implement Symbol.observable");
  });
}
function ue(t) {
  return new v(function(e) {
    for (var n = 0; n < t.length && !e.closed; n++)
      e.next(t[n]);
    e.complete();
  });
}
function le(t) {
  return new v(function(e) {
    t.then(function(n) {
      e.closed || (e.next(n), e.complete());
    }, function(n) {
      return e.error(n);
    }).then(null, lt);
  });
}
function de(t) {
  return new v(function(e) {
    var n, r;
    try {
      for (var o = Z(t), i = o.next(); !i.done; i = o.next()) {
        var a = i.value;
        if (e.next(a), e.closed)
          return;
      }
    } catch (u) {
      n = { error: u };
    } finally {
      try {
        i && !i.done && (r = o.return) && r.call(o);
      } finally {
        if (n)
          throw n.error;
      }
    }
    e.complete();
  });
}
function gt(t) {
  return new v(function(e) {
    ge(t, e).catch(function(n) {
      return e.error(n);
    });
  });
}
function Me(t) {
  return gt(ae(t));
}
function ge(t, e) {
  var n, r, o, i;
  return Xt(this, void 0, void 0, function() {
    var a, u;
    return ct(this, function(s) {
      switch (s.label) {
        case 0:
          s.trys.push([0, 5, 6, 11]), n = Yt(t), s.label = 1;
        case 1:
          return [4, n.next()];
        case 2:
          if (r = s.sent(), !!r.done)
            return [3, 4];
          if (a = r.value, e.next(a), e.closed)
            return [2];
          s.label = 3;
        case 3:
          return [3, 1];
        case 4:
          return [3, 11];
        case 5:
          return u = s.sent(), o = { error: u }, [3, 11];
        case 6:
          return s.trys.push([6, , 9, 10]), r && !r.done && (i = n.return) ? [4, i.call(n)] : [3, 8];
        case 7:
          s.sent(), s.label = 8;
        case 8:
          return [3, 10];
        case 9:
          if (o)
            throw o.error;
          return [7];
        case 10:
          return [7];
        case 11:
          return e.complete(), [2];
      }
    });
  });
}
function pe(t, e, n, r, o) {
  r === void 0 && (r = 0), o === void 0 && (o = !1);
  var i = e.schedule(function() {
    n(), o ? t.add(this.schedule(null, r)) : this.unsubscribe();
  }, r);
  if (t.add(i), !o)
    return i;
}
function pt(t, e) {
  return Y(function(n, r) {
    var o = 0;
    n.subscribe(O(r, function(i) {
      r.next(t.call(e, i, o++));
    }));
  });
}
var fe = Array.isArray;
function be(t, e) {
  return fe(e) ? t.apply(void 0, W([], L(e))) : t(e);
}
function ye(t) {
  return pt(function(e) {
    return be(t, e);
  });
}
function me(t, e, n, r, o, i, a, u) {
  var s = [], c = 0, l = 0, m = !1, M = function() {
    m && !s.length && !c && e.complete();
  }, b = function(p) {
    return c < r ? D(p) : s.push(p);
  }, D = function(p) {
    i && e.next(p), c++;
    var _ = !1;
    $(n(p, l++)).subscribe(O(e, function(T) {
      o == null || o(T), i ? b(T) : e.next(T);
    }, function() {
      _ = !0;
    }, void 0, function() {
      if (_)
        try {
          c--;
          for (var T = function() {
            var E = s.shift();
            a ? pe(e, a, function() {
              return D(E);
            }) : D(E);
          }; s.length && c < r; )
            T();
          M();
        } catch (E) {
          e.error(E);
        }
    }));
  };
  return t.subscribe(O(e, b, function() {
    m = !0, M();
  })), function() {
    u == null || u();
  };
}
function ft(t, e, n) {
  return n === void 0 && (n = 1 / 0), d(e) ? ft(function(r, o) {
    return pt(function(i, a) {
      return e(r, i, o, a);
    })($(t(r, o)));
  }, n) : (typeof e == "number" && (n = e), Y(function(r, o) {
    return me(r, o, t, n);
  }));
}
var xe = ["addListener", "removeListener"], De = ["addEventListener", "removeEventListener"], ze = ["on", "off"];
function R(t, e, n, r) {
  if (d(n) && (r = n, n = void 0), r)
    return R(t, e, n).pipe(ye(r));
  var o = L(Ae(t) ? De.map(function(u) {
    return function(s) {
      return t[u](e, s, n);
    };
  }) : Ne(t) ? xe.map(it(t, e)) : he(t) ? ze.map(it(t, e)) : [], 2), i = o[0], a = o[1];
  if (!i && Mt(t))
    return ft(function(u) {
      return R(u, e, n);
    })($(t));
  if (!i)
    throw new TypeError("Invalid event target");
  return new v(function(u) {
    var s = function() {
      for (var c = [], l = 0; l < arguments.length; l++)
        c[l] = arguments[l];
      return u.next(1 < c.length ? c : c[0]);
    };
    return i(s), function() {
      return a(s);
    };
  });
}
function it(t, e) {
  return function(n) {
    return function(r) {
      return t[n](e, r);
    };
  };
}
function Ne(t) {
  return d(t.addListener) && d(t.removeListener);
}
function he(t) {
  return d(t.on) && d(t.off);
}
function Ae(t) {
  return d(t.addEventListener) && d(t.removeEventListener);
}
function ot(t, e) {
  return Y(function(n, r) {
    var o = 0;
    n.subscribe(O(r, function(i) {
      return t.call(e, i, o++) && r.next(i);
    }));
  });
}
function ve(t, e, n) {
  var r = d(t) || e || n ? { next: t, error: e, complete: n } : t;
  return r ? Y(function(o, i) {
    var a;
    (a = r.subscribe) === null || a === void 0 || a.call(r);
    var u = !0;
    o.subscribe(O(i, function(s) {
      var c;
      (c = r.next) === null || c === void 0 || c.call(r, s), i.next(s);
    }, function() {
      var s;
      u = !1, (s = r.complete) === null || s === void 0 || s.call(r), i.complete();
    }, function(s) {
      var c;
      u = !1, (c = r.error) === null || c === void 0 || c.call(r, s), i.error(s);
    }, function() {
      var s, c;
      u && ((s = r.unsubscribe) === null || s === void 0 || s.call(r)), (c = r.finalize) === null || c === void 0 || c.call(r);
    }));
  }) : dt;
}
var Ie = Object.defineProperty, je = Object.getOwnPropertyDescriptor, x = (t, e, n, r) => {
  for (var o = r > 1 ? void 0 : r ? je(e, n) : e, i = t.length - 1, a; i >= 0; i--)
    (a = t[i]) && (o = (r ? a(e, n, o) : a(o)) || o);
  return r && o && Ie(e, n, o), o;
};
let y = class extends G {
  constructor() {
    super(), this.connected = !1, this.loading = !1, this.showNotification = !1, this.showPopover = !1, this.connecting = !1, this.requestItems = [], this.accounts = [], this.explorer = {
      baseUrl: "https://betanet-dashboard.radixdlt.com/",
      transactionPath: "transaction/",
      accountsPath: "account/"
    }, this.dAppName = "", this.personaLabel = "", this.subscriptions = new C(), this.fontGoogleApiHref = "https://fonts.googleapis.com/css?family=IBM+Plex+Sans:400,600", this.injectFontCSS(), this.subscriptions.add(
      R(window, "click").pipe(
        ot(() => this.showPopover),
        ot((t) => !this.contains(t.target)),
        ve(() => this.showPopover = !1)
      ).subscribe()
    );
  }
  connectedCallback() {
    super.connectedCallback(), this.dispatchEvent(
      new CustomEvent("onRender", {
        bubbles: !0,
        composed: !0
      })
    );
  }
  disconnectedCallback() {
    this.dispatchEvent(
      new CustomEvent("onDestroy", {
        bubbles: !0,
        composed: !0
      })
    );
  }
  shouldSkipFontInjection() {
    return !!document.head.querySelector(
      `link[href|="${this.fontGoogleApiHref}"]`
    ) || document.fonts.check("16px IBM Plex Sans");
  }
  injectFontCSS() {
    if (this.shouldSkipFontInjection())
      return;
    const t = document.createElement("link");
    t.setAttribute("rel", "stylesheet"), t.setAttribute("href", this.fontGoogleApiHref), document.head.append(t);
  }
  togglePopover() {
    this.showPopover = !this.showPopover, this.showPopover && this.dispatchEvent(
      new CustomEvent("onShowPopover", {
        bubbles: !0,
        composed: !0
      })
    );
  }
  formatAccountAddress(t) {
    return `${t.slice(0, 4)}...${t.slice(-6)}`;
  }
  formatAccountLabel(t) {
    return t.length > 14 ? `${t.slice(0, 12).trimEnd()}...` : t;
  }
  connectButtonTemplate() {
    const t = this.connected ? "Connected" : "Connect";
    return g` <radix-button
      ?loading=${this.loading}
      ?gradient=${this.connected}
      @onClick=${this.togglePopover}
      logo
      fullWidth
      >${this.loading ? "" : t}</radix-button
    >`;
  }
  requestItemsTemplate() {
    return this.requestItems.length === 0 ? "" : g`<div class="wrapper request-item-list">
      ${this.requestItems.slice(-3).reverse().map(
      (t) => g`<div class="wrapper request-item">
              <radix-request-item
                .item=${t}
                transactionExplorerBaseUrl="${this.explorer.baseUrl}${this.explorer.transactionPath}"
                @onCancel=${() => {
        this.dispatchEvent(
          new CustomEvent("onCancelRequestItem", {
            detail: { id: t.id },
            bubbles: !0,
            composed: !0
          })
        );
      }}
              ></radix-request-item>
            </div>`
    )}
    </div>`;
  }
  connectTemplate() {
    return !this.connected && !this.connecting ? g`<div class="wrapper connect">
        <div class="wrapper connect-your-wallet">
          <img class="logo" src=${Pt} />
          <span class="text connect">Connect your Radix Wallet</span>
        </div>
        <radix-button
          fullWidth
          @onClick=${() => {
      this.dispatchEvent(
        new CustomEvent("onConnect", {
          bubbles: !0,
          composed: !0
        })
      );
    }}
          >Connect Now</radix-button
        >
        <a
          class="link wallet"
          href=${at.links["What is a radix wallet?"]}
          target="_blank"
          >What is a Radix Wallet?</a
        >
      </div>` : "";
  }
  disconnectWalletTemplate() {
    return this.connected ? g`<div class="wrapper disconnect-wallet">
        <radix-button
          class="button disconnect-wallet"
          text
          @onClick=${() => {
      this.dispatchEvent(
        new CustomEvent("onDisconnect", {
          bubbles: !0,
          composed: !0
        })
      );
    }}
          >Disconnect Wallet</radix-button
        >
      </div>` : "";
  }
  accountListTemplate() {
    return g`<div class="wrapper account-list">
      ${this.accounts.map(
      (t) => g`<div class="wrapper account gradient-${t.appearanceId}">
            <span class="text account-label"
              >${this.formatAccountLabel(t.label)}</span
            >
            <a
              class="text account-address"
              target="_blank"
              href="${this.explorer.baseUrl}${this.explorer.accountsPath}${t.address}"
              >${this.formatAccountAddress(t.address)}<span
                class="icon link"
              ></span
            ></a>
          </div>`
    )}
    </div>`;
  }
  sharedDataTemplate() {
    return this.accounts.length && this.connected ? g`<div class="wrapper sharing">
        <span class="text sharing">You're sharing with ${this.dAppName}:</span>
        ${this.accountListTemplate()}
      </div> ` : "";
  }
  personaTemplate() {
    return this.personaLabel && this.connected ? g`<div class="wrapper persona">
        <span class="icon user"></span>
        <span class="text persona-label">${this.personaLabel}</span>
      </div>` : "";
  }
  popoverTemplate() {
    return this.showPopover ? g`<radix-popover class="popover"
      >${this.requestItemsTemplate()}${this.personaTemplate()}${this.sharedDataTemplate()}${this.disconnectWalletTemplate()}${this.connectTemplate()}</radix-popover
    >` : "";
  }
  notificationTemplate() {
    return this.showNotification ? g`<span class="icon notification"></span>` : "";
  }
  render() {
    return g`
      <div class="wrapper main">
        ${this.connectButtonTemplate()}${this.notificationTemplate()}
        ${this.popoverTemplate()}
      </div>
    `;
  }
};
y.styles = z`
    :host {
      font-family: 'IBM Plex Sans';
      position: relative;
      display: inline-block;
    }
    .popover {
      position: absolute;
      top: calc(100% + 1rem);
      right: 0;
    }
    .wrapper.main {
      width: 8.6rem;
    }
    .wrapper.request-item-list {
      margin-bottom: 1rem;
    }
    .wrapper.request-item {
      margin-bottom: 0.5rem;
    }
    .wrapper.request-item:last-of-type {
      margin-bottom: 0;
    }
    .wrapper.connect {
      margin-bottom: 1rem;
    }
    .wrapper.connect-your-wallet {
      display: flex;
      margin-bottom: 1.5rem;
    }
    .wrapper.account {
      background: #ced2d9;
      display: flex;
      width: 100%;
      padding: 0.5em 0.5em;
      box-sizing: border-box;
      justify-content: space-between;
      margin-bottom: 0.5rem;
      border-radius: 12px;
    }
    .wrapper.persona {
      width: 100%;
      box-sizing: border-box;
      margin-bottom: 0.5rem;
      border-radius: 12px;
      padding: 0.5rem;
      border-radius: 12px;
      background: #e2e5ed;
      display: inline-block;
    }
    .wrapper.sharing {
      margin-bottom: 1rem;
    }
    .wrapper.disconnect-wallet {
      text-align: center;
    }
    .wrapper.persona {
      display: flex;
    }
    .account.gradient-0 {
      background: linear-gradient(276.58deg, #01e2a0 -0.6%, #052cc0 102.8%);
    }
    .account.gradient-1 {
      background: linear-gradient(276.33deg, #ff43ca -14.55%, #052cc0 102.71%);
    }
    .account.gradient-2 {
      background: linear-gradient(276.33deg, #20e4ff -14.55%, #052cc0 102.71%);
    }
    .account.gradient-3 {
      background: linear-gradient(94.8deg, #00ab84 -1.2%, #052cc0 103.67%);
    }
    .account.gradient-4 {
      background: linear-gradient(94.62deg, #ce0d98 -10.14%, #052cc0 104.1%);
    }
    .account.gradient-5 {
      background: linear-gradient(276.33deg, #052cc0 -14.55%, #0dcae4 102.71%);
    }
    .account.gradient-6 {
      background: linear-gradient(90.89deg, #003057 -2.21%, #03d597 102.16%);
    }
    .account.gradient-7 {
      background: linear-gradient(276.23deg, #f31dbe -2.1%, #003057 102.67%);
    }
    .account.gradient-8 {
      background: linear-gradient(276.48deg, #003057 -0.14%, #052cc0 102.77%);
    }
    .account.gradient-9 {
      background: linear-gradient(276.32deg, #1af4b5 -5.15%, #0ba97d 102.7%);
    }
    .account.gradient-10 {
      background: linear-gradient(276.23deg, #e225b3 -2.1%, #7e0d5f 102.67%);
    }
    .account.gradient-11 {
      background: linear-gradient(276.48deg, #1f48e2 -0.14%, #040b72 102.77%);
    }
    .account:last-of-type {
      margin-bottom: 0;
    }
    .text.connect {
      color: ${N.radixGrey1};
      font-style: normal;
      font-size: 1.1rem;
      width: 10rem;
      margin-left: 0.9rem;
      font-weight: 600;
      text-align: left;
    }
    .text.sharing {
      font-size: 1.1rem;
      font-weight: bold;
      margin-bottom: 0.5rem;
      display: inline-block;
    }
    .text.account-label {
      color: white;
      font-weight: 600;
      margin-left: 1rem;
      font-size: 1rem;
      word-break: break-word;
    }
    .text.account-label:first-letter {
      text-transform: uppercase;
    }
    .text.account-address {
      color: white;
      opacity: 0.8;
      margin-right: 1rem;
      font-size: 0.9rem;
      align-self: center;
      white-space: nowrap;
    }
    .text.persona-label {
      align-self: center;
      margin-left: 1rem;
    }
    loading-spinner.small {
      display: inline-block;
      vertical-align: top;
    }
    .logo {
      width: 3rem;
      align-self: center;
    }
    .link.wallet {
      margin-top: 1.2rem;
      text-decoration: none;
      color: ${N.radixBlue};
      font-size: 1rem;
      font-weight: 600;
      display: inline-block;
    }
    .link.wallet::before {
      position: relative;
      top: 0.2rem;
      padding-right: 0.2rem;
      content: url(${h(Et)});
    }
    .button.disconnect-wallet {
      margin-bottom: 0.5rem;
      display: inline-block;
    }
    .icon.user {
      background: white;
      display: inline-block;
      height: 2rem;
      width: 2rem;
      text-align: center;
      border-radius: 50%;
      margin-left: 1rem;
    }
    .icon.user::before {
      position: relative;
      top: 0.05rem;
      content: url(${h(St)});
    }
    .icon.link {
      display: inline-block;
      width: 0.7rem;
      margin-left: 0.3rem;
    }
    .icon.link::before {
      content: url(${h(Wt)});
      align-self: center;
    }
    .icon.notification {
      width: 0.8rem;
      height: 0.8rem;
      background: #f81b1b;
      position: absolute;
      border-radius: 50%;
      right: 0.5rem;
      top: calc(100% - 0.5rem);
    }
  `;
x([
  f({ type: Boolean })
], y.prototype, "connected", 2);
x([
  f({ type: Boolean })
], y.prototype, "loading", 2);
x([
  f({ type: Boolean })
], y.prototype, "showNotification", 2);
x([
  f({
    type: Boolean
  })
], y.prototype, "showPopover", 2);
x([
  f({
    type: Boolean
  })
], y.prototype, "connecting", 2);
x([
  f({ type: Array })
], y.prototype, "requestItems", 2);
x([
  f({ type: Array })
], y.prototype, "accounts", 2);
x([
  f({ type: Object })
], y.prototype, "explorer", 2);
x([
  f({ type: String })
], y.prototype, "dAppName", 2);
x([
  f({ type: String })
], y.prototype, "personaLabel", 2);
y = x([
  P(at.elementTag)
], y);
export {
  y as ConnectButton,
  B as RequestItemType,
  Le as RequestStatus
};
