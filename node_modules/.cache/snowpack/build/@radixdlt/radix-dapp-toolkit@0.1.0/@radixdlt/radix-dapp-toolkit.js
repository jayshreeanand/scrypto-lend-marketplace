...
Bt.fromThrowable;
const bo = (t, e) => e ? !t || !(e != null && e.quantity) || !(e != null && e.quantifier) ? false : e.quantifier === "exactly" ? t.length === e.quantity : t.length >= e.quantity : true, xn = (t, {
  state: e,
  logger: n,
  walletClient: r
}) => {
  if ([
    !!(e != null && e.persona),
    bo(e == null ? void 0 : e.accounts, t.ongoingAccountsWithoutProofOfOwnership)
  ...
  const _ = n.state$.pipe(br(), co()), x = _.pipe(be(({connected: y}) => y), It()), m = _.pipe(be(({persona: y}) => y), It()), D = _.pipe(be(({accounts: y}) => y));
  u.add(_.pipe(N((y) => {
    r == null || r.debug("state$", y);
  })).subscribe()), u.add(D.pipe(N((y) => {
    i.setAccounts(y || []);
  })).subscribe()), u.add(m.pipe(N((y) => {
    i.setPersonaLabel((y == null ? void 0 : y.label) ?? "");
  })).subscribe()), u.add(i.onDisconnect$.pipe(N(() => {
    h(), o.resetRequestItems(), c();
  })).subscribe()), u.add(o.pendingRequests$.pipe(N((y) => {
    i.setLoading(y);
  })).subscribe()), u.add(o.requestItems$.pipe(N((y) => {
    i.setRequestItems(y);
  })).subscribe()), u.add(x.pipe(N((y) => {
    i.setConnected(y);
  })).subscribe()), u.add(n.setState.pipe(so(({state: y, persist: K}) => Fi(y).pipe(uo((q, S) => ({...q, ...S}), $t), He((q) => (r == null || r.debug("stateUpdated", q), n.state$.next(q), K ? p(q) : []))))).subscribe());
  const Z = () => oe.fromPromise(zi(_), (y) => y), W = ({accounts: y}) => Z().andThen((K) => xn(Sn({
  ...
  return u.add(i.onConnect$.pipe(N(() => {
    r == null || r.debug("onConnect");
  }), lo(_), gr(([, y]) => {
    var K;
    return ((K = t.connectRequest) == null ? void 0 : K.call(t, (q) => (r == null || r.debug("connectRequest", q), i.setConnecting(true), xn(Sn({
    ...
    connected$: _.pipe(be(({connected: y}) => y), It()),
  ...
  import('../common/connect-button-e431ab55-7fa91dd7.js');
  const e = t.subjects || xo(), n = t.logger, r = (a) => {
    a();
  }, i = t.onConnect || r, o = () => document.querySelector("radix-connect-button"), s = new Te();
  return s.add(Pe(document, "onRender").pipe(be(() => o()), io((a) => !!a), gr((a) => {
    n == null || n.debug("connectButtonDiscovered"), a.dAppName = t.dAppName, t.explorer && (a.explorer = t.explorer);
    const c = Pe(a, "onConnect").pipe(N(() => {
      i((m) => e.onConnect.next(m));
    })), u = Pe(a, "onDisconnect").pipe(N(() => {
      e.onDisconnect.next();
    })), l = Pe(a, "onDestroy").pipe(N(() => {
      n == null || n.debug("connectButtonRemovedFromDOM");
    })), p = Pe(a, "onCancelRequestItem").pipe(N((m) => {
      const D = m.detail.id;
      n == null || n.debug("onCancelRequestItem", {id: D}), e.onCancelRequestItem.next(D);
    })), f = e.loading.pipe(N((m) => a.loading = m)), h = e.connected.pipe(N((m) => {
      a.connected = m;
    })), g = e.requestItems.pipe(N((m) => {
      a.requestItems = m;
    })), d = e.accounts.pipe(N((m) => {
      a.accounts = m;
    })), _ = e.personaLabel.pipe(N((m) => {
      a.personaLabel = m;
    })), x = e.connecting.pipe(N((m) => {
      a.connecting = m;
    }));
    return ro(c, f, h, g, u, p, d, _, x, l);
  })).subscribe()), {
  ...
  return i.add(o.onChange.pipe(be(() => f()), N((h) => o.items.next(h))).subscribe()), {
    add: a,
    remove: c,
    updateStatus: l,
    reset: u,
  ...
  return o.add(n.items$.pipe(N((a) => {
    n.subjects.pendingItems.next(a.some((c) => c.status === "pending"));
  })).subscribe()), {
    request: i,
    sendTransaction: (a) => {
  ...
  return {
    getData: (n) => oe.fromPromise(t(n), zt).andThen((r) => r ? Sr(r) : ne(void 0)),
    setData: (n, r) => oe.fromPromise(e(n, r), zt)
  };
};
var ko = Object.defineProperty, Io = (t, e, n) => e in t ? ko(t, e, {enumerable: true, configurable: true, writable: true, value: n}) : t[e] = n, it = (t, e, n) => (Io(t, typeof e != "symbol" ? e + "" : e, n), n), Kt = function(t, e) {
...
}, xs = (t) => t.outgoingMessageSubject.pipe(st((e) => ({
...
(function(t) {
  (function(e, n) {
    ...
    function p(d, _, x) {
      ...
      function y() {
        ...
        if (!(typeof window === n || !Z)) {
          ...
          if (typeof S === n)
            try {
              var ee = window.document.cookie, vn = ee.indexOf(encodeURIComponent(Z) + "=");
    ...
    f.getLogger = function(d) {
      ...
      var _ = h[d];
      return _ || (_ = h[d] = new p(d, f.getLevel(), f.methodFactory)), _;
    };
...
const J = Yr.exports, Ss = (t) => t.incomingMessageSubject.pipe(ze((e) => {
  "eventType" in e ? (J.debug(`🔵💬⬇️ message lifecycle event
${JSON.stringify(e, null, 2)}`), t.messageLifeCycleEventSubject.next(e)) : (J.debug(`🔵⬇️ wallet response
${JSON.stringify(e, null, 2)}`), t.responseSubject.next(e));
})), Ts = (t) => t.dispatchEventSubject.pipe(ze(({event: e, payload: n}) => {
  J.debug(`🔵⬆️ wallet request
${JSON.stringify(n, null, 2)}`), window.dispatchEvent(new CustomEvent(e, {
    detail: n
  }));
})), Os = (t) => {
  const e = new rt();
...
const te = {
  ...
  interactionId: e,
  message: n || $s.get(t) || ""
}), js = (t) => he.fromPromise(fs(t), Rs).andThen((e) => e), Ns = (t, e) => t.messageLifeCycleEventSubject.pipe(at((n) => n.interactionId === e)), Ls = (t) => (e) => (n) => {
  const r = new ye();
  e.requestControl && e.requestControl({
    cancelRequest: () => (J.debug(`🔵⬆️❌ wallet request canceled
${JSON.stringify(n, null, 2)}`), r.next())
  });
  const i = r.asObservable().pipe(st(() => ie(wt(Le.canceledByUser, n.interactionId)))), o = t.responseSubject.pipe(at((h) => h.interactionId === n.interactionId), st((h) => "items" in h ? re(h) : ie(h))), s = Lt(o, i).pipe(kn()), a = Ns(t, n.interactionId).pipe(ze((h) => {
    e.eventCallback && e.eventCallback(h.eventType);
  }), ws(o), Hr()), c = a.subscribe(), u = ms(te.extensionDetectionTime).pipe(st(() => ie(wt(Le.missingExtension, n.interactionId)))), l = Lt(u, a).pipe(kn(), at((h) => !("eventType" in h))), p = ls(true).pipe(ze(() => {
    t.outgoingMessageSubject.next(n);
  }), at(() => false)), f = Lt(s, l, p).pipe(ze(() => {
    c.unsubscribe();
  }));
  return js(f);
};
  ...
  _parse(e) {
    ...
    if (this._def.catchall instanceof Se) {
      ...
      for (const u of s) {
        ...
        a.push({
          key: {status: "valid", value: u},
          value: c._parse(new xe(r, l, r.path, u)),
...
const si = I({
  ...
  xa,
  Sa
]), zn = (t) => he.fromPromise(va.parseAsync(t), (e) => e.issues).map(() => t).mapErr((e) => (J.error("🔵💥 invalid wallet request"), J.debug(JSON.stringify(t, null, 2)), J.debug(JSON.stringify(e, null, 2)), wt(Le.walletRequestValidation, t.interactionId))), Kn = (t) => he.fromPromise(Ta.parseAsync(t), (e) => e.issues).map(() => t).mapErr(() => (J.error("🔵💥 invalid wallet response"), wt(Le.walletRequestValidation, t.interactionId))), Vn = (t) => {
...
}), Hn = (t) => re(Object.entries(t).reduce((e, [n, r]) => {
  switch (n) {
...
class Du {
  ...
  takeFromWorktop(e, n) {
    return this.instructions.push(`TAKE_FROM_WORKTOP ${U(e)} ${Ue(n)};`), this.buckets.set(n, this.id_allocator++), this;
  }
  takeFromWorktopByAmount(e, n, r) {
    return this.instructions.push(`TAKE_FROM_WORKTOP_BY_AMOUNT ${We(e)} ${U(n)} ${Ue(r)};`), this.buckets.set(r, this.id_allocator++), this;
  }
  takeFromWorktopByIds(e, n, r) {
    return this.instructions.push(`TAKE_FROM_WORKTOP_BY_IDS ${e} ${U(n)} ${Ue(r)};`), this.buckets.set(r, this.id_allocator++), this;
  }
  returnToWorktop(e) {
    return this.instructions.push(`RETURN_TO_WORKTOP ${Ue(e)};`), this;
  }
  assertWorktopContains(e) {
    return this.instructions.push(`ASSERT_WORKTOP_CONTAINS ${U(e)};`), this;
  }
  assertWorktopContainsByAmount(e, n) {
    return this.instructions.push(`ASSERT_WORKTOP_CONTAINS_BY_AMOUNT ${We(e)} ${U(n)};`), this;
  }
  assertWorktopContainsByIds(e, n) {
    return this.instructions.push(`ASSERT_WORKTOP_CONTAINS_BY_IDS ${e} ${U(n)};`), this;
  }
  popFromAuthZone(e) {
  ...
  createProofFromAuthZone(e, n) {
    return this.instructions.push(`CREATE_PROOF_FROM_AUTH_ZONE ${U(e)} ${ae(n)};`), this.proofs.set(n, this.id_allocator++), this;
  }
  createProofFromAuthZoneByAmount(e, n, r) {
    return this.instructions.push(`CREATE_PROOF_FROM_AUTH_ZONE_BY_AMOUNT ${We(e)} ${U(n)} ${ae(r)};`), this.proofs.set(r, this.id_allocator++), this;
  }
  createProofFromAuthZoneByIds(e, n, r) {
    return this.instructions.push(`CREATE_PROOF_FROM_AUTH_ZONE_BY_IDS ${e} ${U(n)} ${ae(r)};`), this.proofs.set(r, this.id_allocator++), this;
  }
  createProofFromBucket(e, n) {
    return this.instructions.push(`CREATE_PROOF_FROM_BUCKET ${Ue(e)} ${ae(n)};`), this.proofs.set(n, this.id_allocator++), this;
  }
  cloneProof(e, n) {
    return this.instructions.push(`CLONE_PROOF ${ae(e)} ${ae(n)};`), this.proofs.set(n, this.id_allocator++), this;
  }
  dropProof(e) {
    return this.instructions.push(`DROP_PROOF ${ae(e)};`), this;
  }
  callFunction(e, n, r, i) {
    return this.instructions.push(`CALL_FUNCTION ${Ia(e)} "${n}" "${r}" ${i.join(" ")};`), this;
  }
  callMethod(e, n, r) {
    return this.instructions.push(`CALL_METHOD ${Ae(e)} "${n}" ${r.join(" ")};`), this;
  }
  callNativeFunction(e, n, r) {
    return this.instructions.push('CALL_NATIVE_FUNCTION "' + e + '" "' + n + '" ' + r.join(" ") + ";"), this;
  }
  callNativeMethod(e, n, r) {
    return this.instructions.push("CALL_NATIVE_METHOD " + e + ' "' + n + '" ' + r.join(" ") + ";"), this;
  }
  publishPackageWithOwner(e, n, r) {
    return this.instructions.push('PUBLISH_PACKAGE_WITH_OWNER Blob("' + e + '") Blob("' + n + '") ' + r + ";"), this;
  }
  createResource(e, n, r, i) {
    return this.instructions.push(`CREATE_RESOURCE ${e} ${n} ${r} ${i};`), this;
  }
  burnBucket(e) {
    return this.instructions.push(`BURN_BUCKET ${Ue(e)};`), this;
  }
  mintFungible(e, n) {
    return this.instructions.push(`MINT_FUNGIBLE ${U(e)} ${We(n)};`), this;
  }
  withdrawFromAccount(e, n) {
    return this.instructions.push(`CALL_METHOD ${Ae(e)} "withdraw" ${U(n)};`), this;
  }
  withdrawFromAccountByAmount(e, n, r) {
    return this.instructions.push(`CALL_METHOD ${Ae(e)} "withdraw_by_amount" ${We(n)} ${U(r)};`), this;
  }
  withdrawFromAccountByIds(e, n, r) {
    return this.instructions.push(`CALL_METHOD ${Ae(e)} "withdraw_by_ids" ${n} ${U(r)};`), this;
  }
  createProofFromAccount(e, n) {
    return this.instructions.push(`CALL_METHOD ${Ae(e)} "create_proof" ${U(n)};`), this;
  }
  createProofFromAccountByAmount(e, n, r) {
    return this.instructions.push(`CALL_METHOD ${Ae(e)} "create_proof_by_amount" ${We(n)} ${U(r)};`), this;
  }
  createProofFromAccountByIds(e, n, r) {
    return this.instructions.push(`CALL_METHOD ${Ae(e)} "create_proof_by_ids" ${n} ${U(r)};`), this;
  }
  build() {
...
const ru = {
  ...
  return {
    ...Aa({networkId: t, dAppDefinitionAddress: e}, Ls(r.subjects)),
    destroy: i,
...
export { Eu as Array, sa as AuthLoginRequestResponseItemSchema, aa as AuthRequestItemSchema, Iu as Blob, du as Bool, Ue as Bucket, Ae as ComponentAddress, We as Decimal, Ru as EcdsaSecp256k1PublicKey, $u as EcdsaSecp256k1Signature, ju as EddsaEd25519PublicKey, Nu as EddsaEd25519Signature, Tu as Enum, zs as ErrorTypeSchema, ku as Expression, Cu as Hash, yu as I128, pu as I16, mu as I32, vu as I64, hu as I8, Du as ManifestBuilder, cu as Network, Pu as NonFungibleAddress, Zu as NonFungibleId, ui as OneTimeAccountsRequestResponseItemSchema, Ia as PackageAddress, Lu as PreciseDecimal, ae as Proof, Wu as RadixDappToolkit, qe as RequestTypeSchema, U as ResourceAddress, M as ScryptoValueError, Su as String, Au as SystemAddress, Ou as Tuple, ka as TypeId, xu as U128, gu as U16, wu as U32, _u as U64, bu as U8, fu as Unit, ba as WalletAuthorizedRequestResponseItemsSchema, Sa as WalletInteractionFailureResponseSchema, _a as WalletInteractionResponseItemsSchema, Ta as WalletInteractionResponseSchema, va as WalletInteractionSchema, xa as WalletInteractionSuccessResponseSchema, ha as WalletRequestItemsSchema, ga as WalletRequestResponseItemsSchema, lu as WalletSdk, wa as WalletTransactionResponseItemsSchema, ya as WalletUnauthorizedRequestResponseItemsSchema, wt as createSdkError, Le as errorType, ru as login, iu as oneTimeAccounts, ou as oneTimePersonaData, su as ongoingAccounts, au as ongoingPersonaData, qu as requestBuilder, Uu as requestItem, Rs as sdkError, uu as usePersona };
